...
| `version`      | u16   | yes      | Wire version. v0 uses `0`. |
...
### Header constants
- `magic`: chosen constant (set once; never change in v0+).
- `version`: starts at `0`.
...
Exactly one of `FULL_SNAPSHOT` or `DELTA_SNAPSHOT` MUST be set in v0.
...
Notes:
- FULL snapshot can be represented as a set of creates + updates; however in v0 we keep semantics simple:
...
- In v0: decoder MAY reject unknown tags.
- In v1+: decoders SHOULD skip unknown tags using `section_len` for forward compatibility.
...
In v0, `schema` describes each field codec as:
...
- repeated `count` times:
  - `entity_id` (u32)
  - `type_id` (u16)  // optional in v0 if single entity type; keep if needed now
...
**Flexibility rule:** only include `type_id` if you truly need multi-type entities in v0.
If you don’t, drop it from v0 to keep the format minimal.
...
v0 does not define ACK messages; ACKs are out-of-band and transport-specific.
...
Recommended `CodecLimits` (v0 defaults; tune per game):
- `max_packet_bytes`
- `max_sections`
- `max_entities_create`
- `max_entities_update`
...
- v0 decoders reject packets where `version != 0`.
